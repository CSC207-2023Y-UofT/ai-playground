...
public class TestGenerateDatasets {
  ...
  public static void testGenerateCircular() {
    ...
    for (ArrayList<ArrayList<Double>> cluster : dataset) {
      if (cluster.size() != 2) {
        System.out.println(
            "Test failed: Each cluster should contain two ArrayLists for x and y coordinates.");
        return;
      }

      if (cluster.get(0).size() != 1000 + noise * 10
          || cluster.get(1).size() != 1000 + noise * 10) {
  ...
  public static void testGenerateClusters() {
    ...
    if (cluster1.get(0).get(0).equals(cluster2.get(0).get(0))
      ...
      System.out.println(
          "Test failed: The first and second clusters should have different center coordinates.");
      return;
    }
  ...
  public static void testGenerateQuadrantDatasets() {
    int noise = 5;
    ArrayList<ArrayList<ArrayList<Double>>> datasets =
        GenerateDatasets.generateQuadrantDatasets(noise);
    ...
    for (ArrayList<ArrayList<Double>> quadrant : datasets) {
      if (quadrant.size() != 2) {
        System.out.println(
            "Test failed: Each quadrant dataset should contain two ArrayLists for x and y"
                + " coordinates.");
        return;
      }

      if (quadrant.get(0).size() != 500 + noise * 10
          || quadrant.get(1).size() != 500 + noise * 10) {
  ...
  public static void testGenerateSpiralDatasets() {
    int noise = 5;
    ArrayList<ArrayList<ArrayList<Double>>> datasets =
        GenerateDatasets.generateSpiralDatasets(noise);
    ...
    for (ArrayList<ArrayList<Double>> spiral : datasets) {
      if (spiral.size() != 2) {
        System.out.println(
            "Test failed: Each spiral dataset should contain two ArrayLists for x and y"
                + " coordinates.");
        return;
      }
